...
//
// SUMMARY:
// This code creates a TUN interface named "tun0" and waits for packets to
// arrive. When a packet is read from the TUN interface, it prints the number of
// bytes read. You can process the packet data in the buffer as needed.
//

void setup_ip_conf(char *interface, char *local_ip) {
  // 1. ip addr add `local_ip`/24 dev `interface`
  // 2. ip link set tun0 up
  char cmd[256];
  snprintf(cmd, sizeof(cmd), "ip addr add %s/24 dev %s", local_ip, interface);
  system(cmd);
  snprintf(cmd, sizeof(cmd), "ip link set %s mtu %d up", interface, MTU);
  system(cmd);
}

// the encoding will be really easy, we will just replace each occurences of
// 'dog' with 'cat' for the sake of the exercise.
int encode_packet(const uint8_t *plaintext, int plain_len, uint8_t *enc_buf,
                  int enc_buf_size) {
  int enc_len = 0;
  for (int i = 0; i < plain_len && enc_len < enc_buf_size; i++) {
    if (i + 2 < plain_len && plaintext[i] == 'd' && plaintext[i + 1] == 'o' &&
        plaintext[i + 2] == 'g') {
      if (enc_len + 3 > enc_buf_size)
        break; // Check buffer size
      enc_buf[enc_len++] = 'c';
      enc_buf[enc_len++] = 'a';
      enc_buf[enc_len++] = 't';
      i += 2; // Skip 'dog'
...
int decode_packet(const uint8_t *enc_buf, int enc_len, uint8_t *dec_buf,
                  int dec_buf_size) {
  int dec_len = 0;
  for (int i = 0; i < enc_len && dec_len < dec_buf_size; i++) {
    if (i + 2 < enc_len && enc_buf[i] == 'c' && enc_buf[i + 1] == 'a' &&
        enc_buf[i + 2] == 't') {
      if (dec_len + 3 > dec_buf_size)
        break; // Check buffer size
      dec_buf[dec_len++] = 'd';
      dec_buf[dec_len++] = 'o';
      dec_buf[dec_len++] = 'g';
...
void tunnel(int tun_fd, int udp_fd, struct sockaddr_in *peer_addr) {
  ...
  int max_fd = ((tun_fd > udp_fd) ? tun_fd : udp_fd) +
               1; // Check which fd is larger for select -> the lowest fd is the
                  // sucessful call not currently open for the process
  ...
  while (1) {
    ...
    if (select(max_fd, &read_fds, NULL, NULL, NULL) <
        0) { // Wait for either tun_fd or udp_fd to have data to read
      perror("select");
      break;
    }
    ...
    if (FD_ISSET(tun_fd, &read_fds)) { // SEND PATH : OS -> Encode -> peer
      ...
      if (sendto(udp_fd, enc_buf, enc_len, 0, (struct sockaddr *)peer_addr,
                 sizeof(*peer_addr)) < 0) {
        perror("sendto peer");
        break;
      }
    ...
    if (FD_ISSET(udp_fd, &read_fds)) { // RECV PATH : peer -> Decode -> OS
      ...
      ssize_t nread = recvfrom(udp_fd, enc_buf, sizeof(enc_buf), 0,
